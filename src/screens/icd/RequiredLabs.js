...
export default class RequiredLabs extends Component {
    ...
    init_visit = () => {
        this.setState({
            ...
            lab_list: Global.icd_admin_json.labs,
    ...
    select_right_menu = (index) => {
        ...
        for(i = 0; i < lab_list.length; i ++) {
            if(i == index) {
                lab_list[i].clicked = !lab_list[i].clicked;
            } else {
                lab_list[i].clicked = false;
            }
        }
        this.setState({
            lab_list: lab_list,
    ...
    show_alert = () => {
        ...
        var lab_list = this.state.lab_list;
        lab_list[this.state.selected_right_menu_index].clicked = !lab_list[this.state.selected_right_menu_index].clicked;
        this.setState({
            lab_list: lab_list
        });
        ...
        [
            ...
            {text: 'OK', onPress: () => this.delete_lab(this.state.selected_right_menu_index)},
        ],
        { cancelable: true })
    }

    delete_lab = async(index, action_type) => {

        var lab_list = this.state.lab_list;
        lab_list.splice(index, 1);
        this.setState({
            lab_list: lab_list
        });
        Global.icd_admin_json.labs = lab_list;
        this.setState({
            selected_right_menu_index: -1
        });
    }
    ...
    add_new_lab() {
        this.hidden_right_menu();
        this.props.navigation.navigate("SelectLab", {prev_screen: "RequiredLabs"});
    }
        ...
        <SafeAreaView style = {styles.container}>
        {
            ...
            <View style = {styles.menu_bar}>
                ...
                <View style = {{width: '60%', height: '100%', justifyContent: 'center'}}>
                    <Text style = {{fontSize: 18, color: '#ffffff'}} numberOfLines = {1} renderTruncatedFooter = {() => null}>Required Labs</Text>
                </View>
            ...
            <View style = {{width: '100%', height: main_view_height, alignItems: 'center'}} onStartShouldSetResponder={() => this.hidden_right_menu()}>
                ...
                <TouchableOpacity style = {styles.add_button} onPress = {() => this.add_new_lab()}>
