...
export default class PendingVisit extends Component {
    ...
    donotuseopinion = async(index) => {

    }

    setasmaindoctor = async(index) => {

    }

    recordping = async(index) => {
        this.hidden_right_menu();
        Alert.alert("EpationIndex", "Record Ping: " + this.state.json_array[index].caseNumber + " - " + this.state.json_array[index].id);
    }
    ...
    render() {
        ...
        <SafeAreaView style = {styles.container}>
        {
            ...
            <View style = {{width: '100%', height: main_view_height, alignItems: 'center'}} onStartShouldSetResponder={() => this.hidden_right_menu()}>
                <ScrollView style = {{width: '90%'}} showsVerticalScrollIndicator = {false}>
                    <View style = {{paddingBottom: 320}}>
                    {
                        ...
                        <TouchableOpacity key = {index} style = {[styles.item_view, {zIndex: 1000-index}]} onPress = {() => this.visit_master(item)}>
                            ...
                            {
                                item.clicked &&
                                <View style = {[styles.right_menu_view, this.state.prev_screen == "Home" ? {zIndex: 1000-index} : {height: 320, zIndex: 1000}]}>
                                    <TouchableOpacity style = {styles.right_menu_item_view} onPress = {() => this.delete_alert(index)}>
                                        <Text style = {styles.right_menu_text}>Delete</Text>
                                    </TouchableOpacity>
                                {
                                    this.state.prev_screen != "Home" &&
                                    <View>
                                        <TouchableOpacity style = {styles.right_menu_item_view} onPress = {() => this.donotuseopinion(index)}>
                                            <Text style = {styles.right_menu_text}>Do not use opinion</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity style = {styles.right_menu_item_view} onPress = {() => this.setasmaindoctor(index)}>
                                            <Text style = {styles.right_menu_text}>Set as main doctor</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity style = {styles.right_menu_item_view} onPress = {() => this.recordping(index)}>
                ...
                </ScrollView>
            {
                ...
                <View style = {[styles.add_new_view, this.state.add_new_button_click ? {height: 150} : {height: 50}]}>
                {
                    ...
                    <View style = {{width: '100%', height: 100}}>
                        <TouchableOpacity style = {styles.alternative_button} onPress = {() => this.alternate_opinion()}>
                            <View style = {{width: '70%', height: '60%', backgroundColor: '#3a393a', justifyContent: 'center', alignItems: 'center', borderRadius: 5}}>
                                <Text style = {{fontSize: 13, color: '#ffffff'}}>Alternate Opinion</Text>
                            </View>
                    ...
                    <View style = {[{width: '100%', height: 50, justifyContent: 'center', alignItems: 'flex-end'}]}>
                        <TouchableOpacity style = {styles.add_new_button} onPress = {() => this.setState({add_new_button_click: !this.state.add_new_button_click, right_menu_clicked: true})}>
                        {
                            this.state.add_new_button_click &&
