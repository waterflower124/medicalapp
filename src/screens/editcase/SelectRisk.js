...
export default class SelectRisk extends Component {
    ...
    async UNSAFE_componentWillMount() {
        ...
        await fetch(Global.base_url + '/riskref?query=', {
            method: 'GET',
            headers: {
    ...
    go_back() {
        if(this.state.prev_screen == "Risks") {
            this.props.navigation.navigate("Risks");
        } else if(this.state.prev_screen == "CptRisks") {
    ...
    select_risk(item) {
        if(this.state.prev_screen == "Risks") {
            this.props.navigation.navigate("Risks");
            Global.edit_case_json.risks.push(item);
        } else if(this.state.prev_screen == "CptRisks") {
            item.estado = "",
            item.visitNumber = 0;
            Global.cpt_admin_json.surgeryRisk.push(item);
            this.props.navigation.navigate("CptRisks");
        } 
    }
    ...
    search_risk = async() => {
        Keyboard.dismiss();
        var global_risk_list = this.state.global_risk_list;
        var risk_list = [];
        for(i = 0; i < global_risk_list.length; i ++) {
            if(global_risk_list[i].riskDesc.indexOf(this.state.search_text) > -1) {
                risk_list.push(global_risk_list[i]);
            }
        }
        this.setState({
            risk_list: risk_list
        ...
        <SafeAreaView style = {styles.container}>
        {
            ...
            <View style = {styles.menu_bar}>
                ...
                <View style = {{width: '80%', height: '100%', justifyContent: 'center'}}>
                    <Text style = {{fontSize: 18, color: '#ffffff'}}>Select Risk</Text>
                </View>
            ...
            <View style = {{width: '100%', height: main_view_height, alignItems: 'center'}}>
                <View style = {{width: '95%', height: 40, borderColor: '#000000', borderWidth: 1, flexDirection: 'row', marginTop: 10}}>
                    <View style = {{width: '80%', height: '100%'}}>
                        <TextInput style = {styles.search_input_text} placeholder = {'Search Table'} returnKeyType = {'search'} onSubmitEditing = {() => this.search_risk()} onChangeText = {(text) => this.setState({search_text: text})}></TextInput>
                    </View>
                    <TouchableOpacity style = {{width: '20%', height: '100%', justifyContent: 'center', alignItems: 'flex-end'}} onPress = {() => this.search_risk()}>
                ...
                <View style = {{width: '95%', height: main_view_height - 40 - 10}}>
                    <ScrollView style = {{width: '100%'}} showsVerticalScrollIndicator = {false}>
                    {
                        ...
                        <TouchableOpacity key = {index} style = {styles.component_view} onPress = {() => this.select_risk(item)}>
                            <Text style = {styles.component_content_text}>{item.riskDesc}</Text>
                        </TouchableOpacity>
                        )
                    }
                    </ScrollView>
                </View>
                <TouchableOpacity style = {{width: 40, height: 40, position: 'absolute', right: 10, bottom: 10}} onPress = {() => this.setState({new_risk_modal_show: true})}>
                    <Image style = {{width: '100%', height: '100%'}} resizeMode = {'contain'} source={require('../../assets/images/new_case_add.png')}/>
                </TouchableOpacity>
            </View>
            {
                this.state.new_risk_modal_show &&
                ...
                <View style = {{position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', zIndex: 100}}>
                    <View style = {{width: '80%', height: 230, alignItems: 'center', backgroundColor: '#ffffff'}}>
                        ...
                        <View style = {{width: '95%', height: 110, justifyContent: 'space-around', alignItems: 'center', flexDirection: 'row'}}>
                            <TouchableOpacity style = {{width: '45%', height: 40, alignItems: 'center', justifyContent: 'center', backgroundColor: '#445774'}} onPress = {() => this.setState({new_risk_modal_show: false})}>
                                <Text style = {[styles.component_content_text, {color: '#ffffff'}]}>CANCEL</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style = {{width: '45%', height: 40, alignItems: 'center', justifyContent: 'center', backgroundColor: '#445774'}} onPress = {() => this.add_new_risk()}>
