...
export default class EditDiagnosis extends Component {
    ...
    mark_as_wrong = async(index) => {
        if(Global.edit_case_json.diagnoses[index].wrong == "Y") {
            Global.edit_case_json.diagnoses[index].wrong = "";
        } else {
            Global.edit_case_json.diagnoses[index].wrong = "Y";
        }
        Global.edit_case_json.diagnoses[index].clicked = false;
        this.setState({
            diagnoses_list: Global.edit_case_json.diagnoses
        });
    }
    ...
    open_link = async(index) => {
        this.hidden_right_menu()
        var google_link = "https://www.google.com/?q=" + this.state.diagnoses_list[index].diagnoseDesc;
                        ...
                        <View key = {index} style = {{width: '100%', flexDirection: 'row', marginTop: 20, zIndex: 1000-index}}>
                            ...
                            </View>
                            {
                                ...
                                <View style = {[styles.right_menu_view, {zIndex: 1000-index}]}>
                                    ...
                                    <TouchableOpacity style = {styles.right_menu_item} onPress = {() => this.mark_as_wrong(index)}>
                                        <Text style = {styles.right_menu_text}>{item.wrong == "Y" ? "Mark As True" : "Mark As Wrong"}</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style = {styles.right_menu_item} onPress = {() => this.open_link(index)}>
